...
class BookingForm(ModelForm):
    ...
    def clean(self):
        ...
        if start_time and end_time and end_time < start_time: #เช็คว่ามีคนเลือก end_date ที่มีค่ามากกว่า start_date หรือไม่
            self.add_error(
                "start_time",
                "End time cannot be before start time"
            )

        bookings = Booking.objects.filter( #ถูก book ไปในเวลาที่เลือกรึยัง
